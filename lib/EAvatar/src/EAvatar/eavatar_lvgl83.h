 ...
 * 通过自定义 lv_obj + LV_EVENT_DRAW_MAIN 钩子，直接调用 draw_ctx 的
 * draw_polygon / draw_arc 等回调，把 blob body / capsule eyes 画到屏幕。
 *
...
private:
    ...
    /* 局部重绘包围盒半宽/半高（相对 CX_/CY_，像素）。
     * 本体半径 ≤ 43(egg+呼吸)，眼睛最远 |x| ≈ 34 → 46/48 含 AA 余量。 */
    static constexpr int32_t EAVATAR_INV_HALF_W = 46;
    static constexpr int32_t EAVATAR_INV_HALF_H = 48;
    ...
    /* 画 blob body：64 个采样点连成多边形 */
    void drawBody(lv_draw_ctx_t *ctx, const AvatarFrame &f);

    /* 画一只 capsule 眼睛（用 polygon 近似 4 段 cubic） */
    void drawEye(lv_draw_ctx_t *ctx,
