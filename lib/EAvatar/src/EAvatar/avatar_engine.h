...
namespace eavatar
{
  ...
  public:
    ...
    void event(AvatarEvent);
    void setState(AvatarState);
    void setExpression(AvatarExpression);
    void setGaze(float x, float y);
    void setSpeakingLevel(float);
    AvatarState state() const { return state_; }
    AvatarFrame frame() const { return frame_; }

  private:
    AvatarState state_ = AvatarState::Idle, target_state_ = AvatarState::Idle;
    AvatarExpression expression_ = AvatarExpression::Neutral;
    AvatarFrame frame_{};
     ...
     * auto_showing_ 区分"当前状态是自动播放的节目"还是"外部事件设置的语义状态"
     * —— 外部持续状态（Sleep/Thinking/Speaking）不被自动播放收回。 */
