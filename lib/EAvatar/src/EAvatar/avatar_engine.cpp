...
namespace eavatar {
void AvatarEngine::begin(){frame_=AvatarFrame{}; state_=target_state_=AvatarState::Idle; expression_=AvatarExpression::Neutral; state_start_=transition_start_=0; last_ms_=0; blinking_=false; next_blink_=0; auto_hold_=5.0f; auto_index_=0; auto_showing_=false; target_gaze_x_=target_gaze_y_=0; speaking_level_=0;}
...
void AvatarEngine::setState(AvatarState s){auto_showing_=false; /* 状态所有权移交外部；autoPlayTick 自己设状态后会重新置位 */ if(s==state_)return;state_=target_state_=s;transition_start_=last_ms_;state_start_=last_ms_;}
