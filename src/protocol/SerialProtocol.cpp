...
namespace ekeys
{
    ...
    void SerialProtocol::handleTcpLine(const char *line)
    {
        if (line == nullptr)
        {
            return;
        }
        /* handleLine 需要可写缓冲（跳过行首空白），拷贝一份 */
        char buf[kLineBufferSize];
        snprintf(buf, sizeof(buf), "%s", line);
        handleLine(buf);
    }

    void SerialProtocol::sendVoiceText(const char *text)
    {
        /* 参考工程格式：{"cmd":0x0c,"seq":N,"text":"...","timestamp":...} */
        JsonDocument doc;
        doc["cmd"] = CMD_VOICE_TEXT;
        doc["seq"] = 0;
        doc["text"] = text != nullptr ? text : "";
        doc["timestamp"] = millis();
        sendDocument(doc);
    }

    void SerialProtocol::sendMusicControl(const char *action)
    {
        /* 参考工程格式：{"cmd":0x0f,"seq":0,"music_control":{"action":"..."}} */
        JsonDocument doc;
        doc["cmd"] = CMD_MUSIC_CONTROL;
        doc["seq"] = 0;
        doc["music_control"]["action"] = action != nullptr ? action : "";
        sendDocument(doc);
    }
