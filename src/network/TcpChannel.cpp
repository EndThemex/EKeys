...
#include "protocol/commands/cmd_profile.h"
    ...
    void TcpChannel::process()
    {
        ...
        case State::Connecting:
        {
            ...
            if (client->connect(host_ip_, host_port_))
            {
                ...
                /*
                 * 连接成功即推送全部 profile 名称+图标列表（0x10，不含键映射）。
                 * App 据此填充方案选择页；进入键盘设置页后再按 0x05+profile
                 * 单独拉取选中方案的具体配置。此处运行于 loopTask 上下文
                 * （MainTask::loop → process），与协议命令处理同任务，无并发问题。
                 */
                protocol::commands::sendProfileState(0);
            }
